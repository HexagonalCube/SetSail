...
namespace PRISM.Utils { 
    ...
    public class PRISMEffectsEditor : VolumeComponentEditor
    {
        ...
        public Texture2D noChangeLutTex;
        private Color selectedColor = Color.black;
        private Color selectedColorMid = Color.black;
        private Color selectedColorHigh = Color.black;
        ...
        const string cuppLutNochangeStringb64 = "";
        private GUIStyle cuppLabelStyle;
        private GUIStyle cuppLabelStyleSMH_Shadows;
        private GUIStyle cuppLabelStyleSMH_Midtones;
        private GUIStyle cuppLabelStyleSMH_Highlights;
        ...
        SerializedDataParameter bloomDebug;
       ...
       public string currentEffectsString = "";
        ...
        //    [InspectorName("Color Temperature (WarmCool)"), Tooltip("Controls the temperature of the color, used to offset light color for a neutral scene")]
        ...
        #endregion
        // Utility function to create a Texture2D
        ...
        public override void OnEnable()
        {
            ...
            fastMode = Unpack(prism.Find(x => x.fastMode));
            ...
            bloomDebug = Unpack(prism.Find(x => x.bloomDebug));
            ...
            byte[] b64_bytesTwo = System.Convert.FromBase64String(cuppLutNochangeStringb64);

            noChangeLutTex = new Texture2D(1024, 32);
            noChangeLutTex.LoadImage(b64_bytesTwo);
        }
        ...
        public static string ConvertToRGBVal(float a)
        {
            return Mathf.CeilToInt(Mathf.Lerp(0f, 255f, a)).ToString();
        }

        public void PopulateCurrenctEffectsString()
        {
            ...
            {
                currentEffectsString += "Lens Distortion";
                currentEffectsString += ", ";
            }

            if (useFilmicNoise.value.boolValue == true)
            {
                currentEffectsString += "Noise";
                currentEffectsString += ", ";
            }

            if (useDither.value.boolValue == true)
            {
                currentEffectsString += "Dithering";
                currentEffectsString += ", ";
            }

            if (tonemap.value.boolValue == true)
            {
            ...
            if (m_Intensity.value.floatValue == 0f) currentEffectsString = "";
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            fastMode.overrideState.boolValue = true;
            ...
            shadows.overrideState.boolValue = true;
            midtones.overrideState.boolValue = true;
            highlights.overrideState.boolValue = true;

            useBloom.overrideState.boolValue = true;
            bloomDebug.overrideState.boolValue = true;
            bloomStrength.overrideState.boolValue = true;
            bloomThreshold.overrideState.boolValue = true;
            bloomRadius.overrideState.boolValue = true;
            useLensDirt.overrideState.boolValue = true;
            bloomDirtTexture.overrideState.boolValue = true;
            lensDirtExposure.overrideState.boolValue = true;
            increaseBloomClarity.overrideState.boolValue = true;


            tonemap.overrideState.boolValue = true;
            gammaOffset.overrideState.boolValue = true;
            useFilmicNoise.overrideState.boolValue = true;
            ...
            // Initialize the GUIStyle for the label
            cuppLabelStyle = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleCenter, // Centered text
                fontSize = 12, // Adjust font size as needed
                fontStyle = FontStyle.Bold,
                normal = {
                textColor = new Color(1f, 0.5f, 0.5f), // Light red color
                background = MakeTex(2, 2, new Color(0.25f, 0.25f, 0.25f)) // Grey background
            }
            };

            // Initialize the GUIStyle for the label
            cuppLabelStyleSMH_Shadows = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleCenter, // Centered text
                fontSize = 10, // Adjust font size as needed
                normal = {
                textColor = new Color(0.89f, 0.89f, 0.89f), // Light red color
                background = MakeTex(2, 2, new Color(0.05f, 0.05f, 0.05f)) // Grey background
                }
            };

            // Initialize the GUIStyle for the label
            cuppLabelStyleSMH_Midtones = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleCenter, // Centered text
                fontSize = 10, // Adjust font size as needed
                normal = {
                textColor = new Color(0.99f, 0.99f, 0.99f), // Light red color
                background = MakeTex(2, 2, new Color(0.5f, 0.5f, 0.5f)) // Grey background
                }
            };


            // Initialize the GUIStyle for the label
            cuppLabelStyleSMH_Highlights = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleCenter, // Centered text
                fontSize = 10, // Adjust font size as needed
                normal = {
                textColor = new Color(0.19f, 0.19f, 0.19f), // Light red color
                background = MakeTex(2, 2, new Color(0.85f, 0.85f, 0.85f)) // Grey background
                }
            };
            ...
            GUILayout.Label("General Settings", cuppLabelStyle);
            EditorGUILayout.EndVertical();

            EditorGUILayout.BeginHorizontal();
            bool shouldUsePRISM = m_Intensity.value.floatValue == 1f ? true : false;
            shouldUsePRISM = EditorGUILayout.Toggle("Use Cinematic URP Post-Processing", shouldUsePRISM);
            ...
            fastMode.value.boolValue = EditorGUILayout.Toggle("Fast (Mobile) mode", fastMode.value.boolValue);

            //EditorGUILayout.Space();
            PopulateCurrenctEffectsString();
            EditorGUILayout.LabelField("Current effects: " + currentEffectsString);
           ...
           //useLinearSpace.value.boolValue = EditorGUILayout.Toggle(linearSpaceContent, useLinearSpace.value.boolValue);
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            GUILayout.Label("Color Correction", cuppLabelStyle);
            EditorGUILayout.EndVertical();
            ...
            if (useColorCorrection.value.boolValue == true)
            {
                ...
                EditorGUILayout.BeginHorizontal(textAreaOptions);

                //GUILayout.Label("Shadows:", textAreaOptions);
                EditorGUILayout.BeginVertical(GUI.skin.box);
                GUILayout.Label("Shadows", cuppLabelStyleSMH_Shadows);
                EditorGUILayout.EndVertical();



                // Calculate the Rect where the color wheel is drawn
                Rect colorWheelRect = GUILayoutUtility.GetRect(colorWheelSize, colorWheelSize, colorWheelSize, colorWheelSize);
                colorWheelRect.size = new Vector2(128f, 128f);
                // Draw the color wheel
                GUI.DrawTexture(colorWheelRect, colorWheelTex, ScaleMode.ScaleToFit);
                EditorGUILayout.BeginVertical(textAreaOptions);
                // Check if the mouse is clicked within the color wheel rect
                //if (GUILayout.Button(colorWheelTex, GUILayout.Width(256), GUILayout.Height(256)))
                {
                    if ((Event.current.type == EventType.MouseDown || Event.current.type == EventType.MouseDrag) && colorWheelRect.Contains(Event.current.mousePosition, true))
                    {
                        Vector2 pickPosition = Event.current.mousePosition;
                        // Scale mouse position to texture coordinates
                        pickPosition.x -= colorWheelRect.x;
                        //Debug.Log(pickPosition.y);
                        pickPosition.y -= colorWheelRect.y;
                        pickPosition.x = Mathf.Clamp(pickPosition.x, 0, colorWheelRect.width);
                        pickPosition.y = Mathf.Clamp(pickPosition.y, 0, colorWheelRect.height);

                        pickPosition.y = Mathf.Lerp(colorWheelSize, 0f, pickPosition.y / colorWheelSize);

                        // Convert to texture scale
                        float x = pickPosition.x / colorWheelRect.width * colorWheelTex.width;
                        float y = pickPosition.y / colorWheelRect.height * colorWheelTex.height;

                        selectedColor = colorWheelTex.GetPixel((int)x, (int)y);

                        // Use Repaint to immediately refresh the editor window
                        Repaint();
                    }
                    else
                    {
                        selectedColor = shadows.value.colorValue;
                    }
                }

                //GUILayout.Label("Selected Color:");

                float selectedColourIntensity = EditorGUILayout.Slider("Lift: ", shadows.value.colorValue.a, 0f, 1f);
                selectedColor.a = selectedColourIntensity;
                shadows.value.colorValue = EditorGUILayout.ColorField(selectedColor, textAreaOptions);
                GUILayout.Label("RGB Values: " + ConvertToRGBVal(selectedColor.r) + ", " + ConvertToRGBVal(selectedColor.b) + ", " + ConvertToRGBVal(selectedColor.g), textAreaOptions);
                //EditorGUILayout.LabelField("R: " + selectedColor.r);
                //EditorGUILayout.LabelField("G: " + selectedColor.g);
                //EditorGUILayout.LabelField("B: " + selectedColor.b);

                //GUILayout.Label("Intensity:");
                //float intensity = selectedColor.maxColorComponent; // You can modify this calculation as needed
                //EditorGUILayout.FloatField(intensity);
                ...
                //GUILayout.Label("Midtones:", textAreaOptions);

                EditorGUILayout.BeginVertical(GUI.skin.box);
                GUILayout.Label("Midtones", cuppLabelStyleSMH_Midtones);
                EditorGUILayout.EndVertical();


                // Calculate the Rect where the color wheel is drawn
                Rect colorWheelRectMidtones = GUILayoutUtility.GetRect(colorWheelSize, colorWheelSize, colorWheelSize, colorWheelSize);
                colorWheelRectMidtones.size = new Vector2(128f, 128f);
                // Draw the color wheel
                GUI.DrawTexture(colorWheelRectMidtones, colorWheelTex, ScaleMode.ScaleToFit);

                EditorGUILayout.BeginVertical(textAreaOptions);
                // Check if the mouse is clicked within the color wheel rect
                //if (GUILayout.Button(colorWheelTex, GUILayout.Width(256), GUILayout.Height(256)))
                {
                    if ((Event.current.type == EventType.MouseDown || Event.current.type == EventType.MouseDrag) && colorWheelRectMidtones.Contains(Event.current.mousePosition, true))
                    {
                        Vector2 pickPosition = Event.current.mousePosition;
                        // Scale mouse position to texture coordinates
                        pickPosition.x -= colorWheelRect.x;
                        //Debug.Log(pickPosition.y);
                        pickPosition.y -= colorWheelRectMidtones.y;
                        pickPosition.x = Mathf.Clamp(pickPosition.x, 0, colorWheelRectMidtones.width);
                        pickPosition.y = Mathf.Clamp(pickPosition.y, 0, colorWheelRectMidtones.height);

                        pickPosition.y = Mathf.Lerp(colorWheelSize, 0f, pickPosition.y / colorWheelSize);

                        // Convert to texture scale
                        float x = pickPosition.x / colorWheelRectMidtones.width * colorWheelTex.width;
                        float y = pickPosition.y / colorWheelRectMidtones.height * colorWheelTex.height;

                        selectedColorMid = colorWheelTex.GetPixel((int)x, (int)y);
                    ...
                    {
                        selectedColorMid = midtones.value.colorValue;
                    }
                }


                //GUILayout.Label("Selected Color:", textAreaOptions);
                float selectedColourIntensityMidtones = EditorGUILayout.Slider("Gamma: ", midtones.value.colorValue.a, 0f, 1f);
                selectedColorMid.a = selectedColourIntensityMidtones;
                midtones.value.colorValue = EditorGUILayout.ColorField(selectedColorMid, textAreaOptions);
                GUILayout.Label("RGB Values: " + ConvertToRGBVal(selectedColorMid.r) + ", " + ConvertToRGBVal(selectedColorMid.b) + ", " + ConvertToRGBVal(selectedColorMid.g), textAreaOptions);
                ...
                //GUILayout.Label("Highlights:", textAreaOptions);

                EditorGUILayout.BeginVertical(GUI.skin.box);
                GUILayout.Label("Highlights", cuppLabelStyleSMH_Highlights);
                EditorGUILayout.EndVertical();


                // Calculate the Rect where the color wheel is drawn
                Rect colorWheelRectHighlights = GUILayoutUtility.GetRect(colorWheelSize, colorWheelSize, colorWheelSize, colorWheelSize);
                colorWheelRectHighlights.size = new Vector2(128f, 128f);
                // Draw the color wheel
                GUI.DrawTexture(colorWheelRectHighlights, colorWheelTex, ScaleMode.ScaleToFit);

                EditorGUILayout.BeginVertical(textAreaOptions);
                // Check if the mouse is clicked within the color wheel rect
                //if (GUILayout.Button(colorWheelTex, GUILayout.Width(256), GUILayout.Height(256)))
                {
                    if ((Event.current.type == EventType.MouseDown || Event.current.type == EventType.MouseDrag) && colorWheelRectHighlights.Contains(Event.current.mousePosition, true))
                    {
                        Vector2 pickPosition = Event.current.mousePosition;
                        // Scale mouse position to texture coordinates
                        pickPosition.x -= colorWheelRectHighlights.x;
                        //Debug.Log(pickPosition.y);
                        pickPosition.y -= colorWheelRectHighlights.y;
                        pickPosition.x = Mathf.Clamp(pickPosition.x, 0, colorWheelRectHighlights.width);
                        pickPosition.y = Mathf.Clamp(pickPosition.y, 0, colorWheelRectHighlights.height);

                        pickPosition.y = Mathf.Lerp(colorWheelSize, 0f, pickPosition.y / colorWheelSize);

                        // Convert to texture scale
                        float x = pickPosition.x / colorWheelRectHighlights.width * colorWheelTex.width;
                        float y = pickPosition.y / colorWheelRectHighlights.height * colorWheelTex.height;

                        selectedColorHigh = colorWheelTex.GetPixel((int)x, (int)y);
                    ...
                    {
                        selectedColorHigh = highlights.value.colorValue;
                    }
                }


                //GUILayout.Label("Selected Color:", textAreaOptions);
                float selectedColourIntensityHighlights = EditorGUILayout.Slider("Gain: ", highlights.value.colorValue.a, 0f, 1f);
                selectedColorHigh.a = selectedColourIntensityHighlights;
                highlights.value.colorValue = EditorGUILayout.ColorField(selectedColorHigh, textAreaOptions);
                GUILayout.Label("RGB Values: " + ConvertToRGBVal(selectedColorHigh.r) + ", " + ConvertToRGBVal(selectedColorHigh.b) + ", " + ConvertToRGBVal(selectedColorHigh.g), textAreaOptions);
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            GUILayout.Label("Color Correction LUTs", cuppLabelStyle);
            EditorGUILayout.EndVertical();
            //Lut
            EditorGUILayout.BeginHorizontal();
            ...
            if (useLut.value.boolValue == true)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    Debug.Log("Converting LUT");
                }

                if(!twoDLookupTex.value.objectReferenceValue)
                {
                    twoDLookupTex.value.objectReferenceValue = noChangeLutTex;
                    //Debug.Log(twoDLookupTex.value.objectReferenceValue);
                   // Debug.Log(noChangeLutTex.width);
                }
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            GUILayout.Label("Bloom", cuppLabelStyle);
            EditorGUILayout.EndVertical();
            ...
            if(useBloom.value.boolValue == true)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                bloomDebug.value.boolValue = EditorGUILayout.Toggle(bloomDebugTextureContent, bloomDebug.value.boolValue);
                EditorGUILayout.EndHorizontal();
            }
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            GUILayout.Label("Lens Distortions", cuppLabelStyle);
            EditorGUILayout.EndVertical();
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            GUILayout.Label("Noise & Dithering", cuppLabelStyle);
            EditorGUILayout.EndVertical();
            ...
            EditorGUILayout.BeginVertical(GUI.skin.box);
            GUILayout.Label("RomB Tonemapping", cuppLabelStyle);
            EditorGUILayout.EndVertical();
