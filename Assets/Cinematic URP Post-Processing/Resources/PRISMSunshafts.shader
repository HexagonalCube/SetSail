...
#pragma multi_compile NOT_ULTRA_QUALITY_SUNSHAFTS ULTRA_QUALITY_SUNSHAFTS
...
//
	...
	float4 _Intermediate_TexelSize;
...
#define HALF_MAX 65504.0
	...
	{
		return UNITY_SAMPLE_TEX2D(_Intermediate, uv);
	}
	...
	float4 fragSunBlur(PostProcessVaryings input) : SV_Target
	{
		...
		float rand = gnoise(uv, (float2)0.0);
		//float rand = interleavedGradientNoise(uv / _Intermediate_TexelSize.xy) - 0.5;
		...
		{
			uv -= deltaTexCoord;
			uv += rand * (_Intermediate_TexelSize.xy * 3.);
