...
//1
	...
	float4 MedianSharpen(PostProcessVaryings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
	...
	float4 MedianDepthSharpen(PostProcessVaryings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
	...
	SubShader
	{
		...
		Name "PRISM Median Sharpen"
			...
			HLSLPROGRAM
			#pragma vertex FullScreenTrianglePostProcessVertexProgram
			#pragma fragment MedianSharpen
			ENDHLSL

		}

		Pass{
		Name "PRISM MedianDepthSharpen"
			ZWrite Off
			ZTest Always
			Cull Off
			...
			HLSLPROGRAM
			#pragma vertex FullScreenTrianglePostProcessVertexProgram
			#pragma fragment MedianDepthSharpen
			ENDHLSL
